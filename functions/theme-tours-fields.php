 ...
 * 
 * 
 * foreach ($meta_box_tour['fields'] as $custom_field){
...
$meta_box_tour = array(
	...
	array(
			'name' => '',
			'desc' => '',
			'id' => $prefix . 'upload_image2_button',
			'type' => 'button',
			'std' => 'Browse'
		),
...
add_action('admin_menu', 'tk_add_tour_box');
...
function tk_add_tour_box() {
	global $meta_box_tour, $meta_box_tour_video; 
	add_meta_box($meta_box_tour['id'], $meta_box_tour['title'], 'tk_show_tour_box', $meta_box_tour['page'], $meta_box_tour['context'], $meta_box_tour['priority']);
}
...
function tk_show_tour_box() {
	global $meta_box_tour, $post;
 	
	
	// Use nonce for verification
	echo '<input type="hidden" name="tk_meta_box_tour_nonce" value="', wp_create_nonce(basename(__FILE__)), '" />';
...
add_action('save_post', 'tk_tour_save_data');
...
function tk_tour_save_data($post_id) {
	global $meta_box_tour, $meta_box_tour_video;
 
	// verify nonce
	if (!wp_verify_nonce($_POST['tk_meta_box_tour_nonce'], basename(__FILE__))) {
		return $post_id;
	}
...
add_filter('is_protected_meta', 'tour_is_protected_meta_filter', 10, 2);
function tour_is_protected_meta_filter($protected, $meta_key) {
